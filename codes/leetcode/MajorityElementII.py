...
class MooreMajorityVoting():
    ...
    def find_two_majority(self):
        ...
        print(candidate1)
        print(candidate2)
        candidate1 = candidate1 if vote_count1 > (len(self.votes)//3) else None
        candidate2 = candidate2 if vote_count2 > (len(self.votes)//3) else None
...
class Solution(object):
    def majorityElement(self, nums):
        ...
        :rtype: List[int]
        """
        candidate1, candidate2 = MooreMajorityVoting(nums).find_two_majority()
        print(candidate1)
        print(candidate2)
        candidates = []
        if not candidate1 == None:
            candidates.append(candidate1)
        if not candidate2 == None:
