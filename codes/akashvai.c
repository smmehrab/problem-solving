...
void showTheScore() {
	...
	iText(board.x + 500, 3 * (board.height / 4), player1.scoreString, GLUT_BITMAP_HELVETICA_18);

	sprintf_s(player2.scoreString, "Player 2 : %d", player2.score);
	iText(board.x + 500, (board.height / 4), player2.scoreString, GLUT_BITMAP_HELVETICA_18);
}
...
void iKeyboard(unsigned char key)
{
	if (selectPos==howToPlay && key == 'b') menu = mainm;
	...
	else if (key == 'w' && (menu == mainm || menu == pause)){
		...
		if (menu==mainm){
			...
			else if (selectPos == restartMatch) selectPos == quickMatch;
